...
const HeaderItemSchema = Joi.object({
  ...
  value: Joi.string().max(parseInt(MAX_HEADER_VALUE_SIZE)).required()
});

const RequestSchema = Joi.object({
  uri: Joi.string().uri().required(),
  method: Joi.string().valid(['post', 'get']).default('post'),
  body: Joi.string().max(parseInt(MAX_RESPONSE_SIZE)).default('{}'),
  headers: Joi.array().items(HeaderItemSchema).max(parseInt(MAX_HEADERS)).min(1)
    .default([
      {
...
const ResponseSchema = Joi.object({
  ...
  headers: Joi.array().items(HeaderItemSchema).max(parseInt(MAX_HEADERS)).min(1).required()
});
...
const TaskSchema = Joi.object({
  ...
  callback: RequestSchema.required(),
});
